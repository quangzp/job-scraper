...
class TopDevExtractor(BaseExtractor):
    ...
    async def try_process_raw_html(self, context: PlaywrightCrawlingContext) -> bool:
        ...
        if response.status < 200 or response.status >= 300:
            logger.info(f"TopDev raw HTML request returned status={response.status}; falling back to Playwright.")
            return False
        if content_type and 'text/html' not in content_type.lower():
            logger.info(f"TopDev raw HTML request returned content-type={content_type}; falling back to Playwright.")
            return False

        html = await response.text()
        soup = BeautifulSoup(html, 'lxml')
        data = self._data_from_soup(soup, self.bs_selectors, context.request.url)
        if not data['title']:
